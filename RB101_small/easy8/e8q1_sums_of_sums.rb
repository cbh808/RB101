...
# Alg
  # define method w/ 1 arg (arr)
  # init loc var `sum` and assign value 0
  # iterate over each element of arr w/ access to each index of arr
  # add to `sum` the sum of array from idx 0 to current idx
  # return `sum`

def sum_of_sums2(arr)
  sum = 0
  arr.each_index {|idx| sum += arr[0..idx].sum}
  sum
end
